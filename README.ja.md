...
- プロンプト行のグループ化 — 行をカテゴリ（Quality、Character、Hair、Clothingなど）に割り当て。複数選択モードまたは行ごとのバッジクリックで設定
- セクション単位の選択モード — Positive/Negativeそれぞれ独立した選択。アクションバー（Set Group / ON / OFF / Ungroup / All / None）
- アウトラインビュー — グループ別の構造表示、折りたたみ、グループ単位ON/OFF、グループヘッダー選択
- 辞書プリセット — グループを名前付きプリセットとして保存、既存グループをプリセットで差し替え（既存行OFF、プリセット行ON）
- プロンプト注釈 — タグに説明を付与（例：`masterpiece` → 「最高品質指定」）、各行の下にインライン表示、全セッション共通
- 一括注釈登録 — 未登録タグを抽出、外部AIで翻訳、結果を一括インポート
- 自動グルーピング — 過去のグループ割り当て履歴に基づき、新規セッションでタグを自動的にグループに振り分け
- 自動保存（localStorage）

### v2（予定）

- 辞書閲覧UI（検索・フィルタ・プリセット管理）
- プリセットのパス形式整理（例：「Misaki/casual」「Misaki/formal」）
- 辞書バックアップ用JSONエクスポート / インポート
...
- プロンプトタグに注釈を付けて自分だけの用語集を構築（全セッション共通で表示）
- 外部AIで未知のタグを一括翻訳し、注釈として一括登録
- 自動グルーピング — 一度グループを設定したタグは新しいセッションでも自動的に分類
